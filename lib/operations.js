...
function getPrinterAttributes(printer, req, res) {
  ...
  console.log(`requested: ${JSON.stringify(requested, null, 3)}`);
...
function getJobs(printer, req, res) {
  var attributes = utils.getAttributesForGroup(
    req._body,
    C.OPERATION_ATTRIBUTES_TAG
  );
  ...
  switch (which) {
    ...
    case undefined:
      ...
      break;
    default:
      res.send(C.CLIENT_ERROR_ATTRIBUTES_OR_VALUES_NOT_SUPPORTED, {
        tag: C.UNSUPPORTED_ATTRIBUTES_TAG,
        attributes: [{ tag: C.UNSUPPORTED, name: 'which-jobs', value: which }],
      });
      return;
  }

  var jobs = states
    ? printer.jobs.filter(function(job) {
        return ~states.indexOf(job.state);
      })
    : printer.jobs;
  ...
  if (_groups[0]) {
    var group = groups.unsupportedAttributesTag(
      _groups[0].attributes,
      requested
    );
