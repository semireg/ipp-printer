...
module.exports = function(printer) {
  ...
  printer.destroyPrinter = function() {
    printer.stop();
    bonjour.unpublishAll(() => {
      bonjour.destroy();
    });
    server.destroy();
    // console.log('Destroyed existing virtual printer.');
  };
  printer.bonjour = bonjour;
  return printer;

  function onrequest(req, res) {
    // console.log('HTTP request: %s %s', req.method, req.url);
  ...
  function onlistening() {
    ...
    if (!printer.uri)
      printer.uri = 'ipp://' + printer.host + ':' + printer.port + '/';

    // console.log('printer "%s" is listening on %s', printer.name, printer.uri);
    printer.start();

    if (printer._zeroconf) {
      debug(
        'advertising printer "%s" on network on port %s',
        printer.name,
        printer.port
      );
      // bonjour.publish({ type: 'ipp', port: printer.port, name: printer.name })
      bonjour.publish({
        type: printer.bonjourType || 'ipp',
        ip: printer.host,
        port: printer.port,
        name: printer.name,
        host: printer.host,
